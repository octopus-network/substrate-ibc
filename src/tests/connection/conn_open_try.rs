...
// 	//     /// Moves the given message into another one, and updates the `previous_connection_id`
// 	// field.     pub fn with_previous_connection_id(
// 	//         self,
...
//         // The proofs in this message are created at a height which the client on destination
// chain does not have.         let msg_proof_height_missing =
//             MsgConnectionOpenTry::try_from(get_dummy_raw_msg_conn_open_try(
...
//             },
//             Test {
//                 name: "Processing fails because the client misses the consensus state targeted by
// the proof".to_string(),                 ctx:
// context.clone().with_client(&msg_proof_height_missing.client_id, Height::new(0,
// client_consensus_state_height).unwrap()),                 msg:
// ConnectionMsg::ConnectionOpenTry(Box::new(msg_proof_height_missing)),                 want_pass:
// false,             },
//             Test {
//                 name: "Good parameters (no previous_connection_id)".to_string(),
//                 ctx: context.clone().with_client(&msg_conn_try.client_id, Height::new(0,
// client_consensus_state_height).unwrap()),                 msg:
// ConnectionMsg::ConnectionOpenTry(Box::new(msg_conn_try.clone())),                 want_pass:
// true,             },
//             Test {
//                 name: "Good parameters".to_string(),
//                 ctx: context.with_client(&msg_conn_try.client_id, Height::new(0,
// client_consensus_state_height).unwrap()),                 msg:
// ConnectionMsg::ConnectionOpenTry(Box::new(msg_conn_try)),                 want_pass: true,
//             },
//         ]
//         .into_iter()
//         .collect();
