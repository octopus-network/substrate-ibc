use crate::{mock::*, ChannelOrder, ClientType, ConsensusState, Error};
...
fn create_client_func() {
    ...
    assert_ok!(IbcModule::create_client(
        identifier1,
        ClientType::GRANDPA,
        height.clone(),
    ...
    assert_err!(
        IbcModule::create_client(
            identifier1,
            ClientType::GRANDPA,
            height.clone(),
            consensus_state
        ),
        Error::<Test>::ClientIdExist
    );
}

#[test]
fn create_client_should_work() {
    new_test_ext().execute_with(|| create_client_func());
}
...
fn bind_port_func() {
    ...
    assert_err!(
        IbcModule::bind_port(identifier.clone(), module_index),
        Error::<Test>::PortIdBinded
    );
}
...
fn chan_open_init_func() {
	...
	assert_err!(IbcModule::chan_open_init(
		module_index,
		order.clone(),
		connection_hops.clone(),
		port_identifier.clone(),
