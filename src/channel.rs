...
impl<T: Config> ChannelReader for Context<T> {
	...
	fn channel_end(&self, port_channel_id: &(PortId, ChannelId)) -> Result<ChannelEnd, ICS04Error> {
		...
		log::info!("in channel : [channel_end] >> port_id = {:?}, channel_id = {:?}", port_channel_id.0, port_channel_id.1);
	...
	fn connection_channels(
		...
		return if <ChannelsConnection<T>>::contains_key(conn_id.as_bytes()) {
			...
			log::info!("in channel : [connection_channels] >> Vector<(PortId, ChannelId)> =  {:?}", result);
			Ok(result)
		} else {
	...
	fn client_consensus_state(
		...
		log::info!("in channel : [client_consensus_state] >> client_id = {:?}, height = {:?}", client_id, height);
		...
		for item in value.iter() {
			if item.0 == height {
				...
				log::info!("in channel: [client_consensus_state] >> any consensus state = {:?}", any_consensus_state);
	...
	fn get_next_sequence_send(
		...
		log::info!("in channel : [get_next_sequence] >> port_id = {:?}, channel_id =  {:?}",
		...
		if <NextSequenceSend<T>>::contains_key(
			...
			log::info!("in channel : [get_next_sequence] >> read get next sequence send return None");
	...
	fn get_next_sequence_recv(
		...
		log::info!("in channel : [get_next_sequence_recv] >> port_id = {:?}, channel_id = {:?}", port_channel_id.0, port_channel_id.1);
		...
		if <NextSequenceRecv<T>>::contains_key(
			...
			log::info!("in channel : [get_next_sequence_recv] >> read get next sequence recv return None");
	...
	fn get_next_sequence_ack(
		...
		log::info!("in channel : [get_next_sequence_ack] >> port_id = {:?}, channel_id = {:?}", port_channel_id.0, port_channel_id.1);
		...
		if <NextSequenceAck<T>>::contains_key(
			...
			log::info!("in channel : [get_next_sequence_ack] >> read get next sequence ack return None");
	...
	fn get_packet_commitment(
		...
		if <PacketCommitment<T>>::contains_key((key.0.as_bytes(), key.1.as_bytes(), seq.clone())) {
			...
			log::info!("in channel : [get_packet_commitment] >> packet_commitment = {:?}", String::from_utf8(data.clone()).unwrap());
			Ok(String::from_utf8(data).unwrap())
		} else {
			log::info!("in channel : [get_packet_commitment] >> read get packet commitment return None");
	...
	fn get_packet_acknowledgement(
		...
		if <Acknowledgements<T>>::contains_key((key.0.as_bytes(), key.1.as_bytes(), seq.clone())) {
			...
			let mut data: &[u8] = &data;
			let data = Vec::<u8>::decode(&mut data).unwrap();
			log::info!("in channel : [get_packet_acknowledgement] >> packet_acknowledgement = {:?}", String::from_utf8(data.clone()).unwrap());
			Ok(String::from_utf8(data).unwrap())
		} else {
			log::info!("in channel : [get_packet_acknowledgement] >> get acknowledgement not found");
	...
	fn host_height(&self) -> Height {
		...
		log::info!("in channel: [host_height] >> host_height = {:?}", Height::new(0, current_height.unwrap()));
	...
	fn channel_counter(&self) -> Result<u64, ICS04Error> {
		...
		log::info!("in channel: [channel_counter] >> channel_counter = {:?}", <Pallet<T> as Store>::ChannelCounter::get());
...
impl<T: Config> ChannelKeeper for Context<T> {
	fn store_packet_commitment(
		...
		log::info!("in channel: [store_packet_commitment] >> port_id = {:?}, channel_id = {:?}, \
		sequence = {:?}, timestamp = {:?}, height  = {:?}, data = {:?}", key.0, key.1, key.2, timestamp, heigh, data);
	...
	fn delete_packet_commitment(
		...
		log::info!("in channel: [delete_packet_commitment] >> port_id = {:?}, channel_id = {:?}, \
		sequence = {:?}", key.0, key.1, key.2);
	...
	fn store_packet_receipt(
		...
		log::info!("in channel: [store_packet_receipt] >> port_id = {:?}, channel_id = {:?},\
		 sequence = {:?}, receipt = {:?}", key.0, key.1, key.2, receipt);
	...
	fn store_packet_acknowledgement(
		...
		log::info!("in channel: [store_packet_acknowledgement] >> port_id = {:?}, channel_id = {:?},\
		 sequence = {:?}, ack = {:?}", key.0, key.1, key.2, ack);
	...
	fn delete_packet_acknowledgement(
		...
		log::info!("in channel: [delete_packet_acknowledgement] >> port_id = {:?}, channel_id = {:?},\
		 sequence = {:?}", key.0, key.1, key.2);
	...
	fn store_connection_channels(
		...
		log::info!("in channel: [store_connection_channels] >> connection_id = {:?}, port_id = {:?},\
		 channel_id = {:?}", conn_id, port_channel_id.0, port_channel_id.1);
	...
	fn store_channel(
		...
		log::info!("in channel: [store_channel] >> port_id = {:?}, channel_id = {:?}, \
			channel_end = {:?}", port_channel_id.0, port_channel_id.1, channel_end);
		...
		<ChannelsKeys<T>>::try_mutate(|val| -> Result<(), &'static str> {
			if let Some(_value) = val.iter().find(|&x| x == &(port_channel_id.0.as_bytes().to_vec(), port_channel_id.1.as_bytes().to_vec())) {
	...
	fn store_next_sequence_send(
		...
		log::info!("in channel: [store_next_sequence_send] >> port_id = {:?}, channel_id = {:?},\
		 sequence = {:?}", port_channel_id.0, port_channel_id.1, seq);
	...
	fn store_next_sequence_recv(
		...
		log::info!("in channel: [store_next_sequence_recv] >> port_id = {:?}, channel_id = {:?}, \
			seq = {:?}", port_channel_id.0, port_channel_id.1, seq);
	...
	fn store_next_sequence_ack(
		...
		log::info!("in channel: [store_next_sequence_ack] >> port_id = {:?}, channel_id = {:?},\
		 sequence = {:?}", port_channel_id.0, port_channel_id.1, seq);
