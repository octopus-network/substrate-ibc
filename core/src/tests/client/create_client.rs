			...
			for msg in create_client_msgs {
				...
				match output {
					Ok(_) => {
						...
						// 	assert_eq!(create_res.client_type,
						// create_res.client_state.client_type()); 	assert_eq!(create_res.client_id,
						// expected_client_id); 	assert_eq!(create_res.client_state.as_ref().
						// clone_into(), msg.client_state); 	assert_eq!(
